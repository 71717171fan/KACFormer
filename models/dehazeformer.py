...
class DehazeFormer(nn.Module):
	def __init__(self, in_chans=3, out_chans=4, window_size=8,
		...
		self.fusion1 = Skip(embed_dims[3],embed_dims[3])
		self.fusion2 = Skip(embed_dims[4],embed_dims[4])
		self.conv1 = nn.Conv2d(24,24,3,1,1)
		self.conv2 = nn.Conv2d(48,48,3,1,1)

		self.corse_conv1 = nn.Sequential(Depthwise_separable_conv(24, 24, kernel_size=3, padding=1, bias=False))
		self.corse_conv2 = nn.Sequential(Depthwise_separable_conv(48, 48, kernel_size=3, padding=1, bias=False))
		self.corse_conv3 = nn.Sequential(Depthwise_separable_conv(96, 96, kernel_size=3, padding=1, bias=False))
		...
		self.relu = nn.ReLU()
		# self.combine_convs3 = nn.Sequential(
	...
	def forward_features(self, x):
		...
		# x = x + self.o1
		# x = self.corse_conv1(x) + x
		# y = self.corse_conv1(self.o1) + self.o1
		# x = self.combine_convs1(x+y)
		skip1 = x
		x = self.layer1(x)

		x = self.patch_merge1(x)
		# x = x + self.o2
		# x = self.corse_conv2(x) + x
		# y = self.corse_conv2(self.o2) + self.o2
		# x = self.combine_convs2(x+y)
		skip2 = x
		x = self.layer2(x)
		...
		# x = x + self.o3
		# x = self.corse_conv3(x) + x
		# y = self.corse_conv3(self.o3) + self.o3
		# x = self.combine_convs3(x+y)
		# skip3 = x
		x = self.layer3(x)
		x = self.patch_split1(x)

		# x = self.fusion1([x, self.skip2(skip2)]) + x
		# x = self.fusion1(x, skip2) + x

		# x = self.corse_conv2(x) +x
		skip21 = self.relu(self.conv2(skip2))+skip2
		skip2 = self.corse_conv2(skip21)+skip2
		# skip2 = self.corse_conv2(skip21)
		x = self.combine_convs2(torch.cat((x,skip2),dim=1))+x
		...
		# x = self.fusion2(x, skip1) + x
		# x = self.corse_conv1(x)+x
		skip11 = self.relu(self.conv1(skip1))+skip1
		skip1 = self.corse_conv1(skip11)+skip1
		x= self.combine_convs1(torch.cat((x,skip1),dim=1))+x
		# skip5 = x
		x = self.layer5(x)
		x = self.patch_unembed(x)
		return x,skip1,skip2,skip4
		# return x

	# def forward(self, x,o1,o2,o3,out):
	def forward(self, x):
		# self.o1 = o1
		# self.o2 = o2
		# self.o3 = o3
		...
		feat,p1,p2,p3= self.forward_features(x)
