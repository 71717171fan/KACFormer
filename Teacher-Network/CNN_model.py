...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv11 = nn.Conv2d(192, 192, 3, 1, 1)
        self.conv22 = nn.Conv2d(96, 96, 3, 1, 1)
        self.conv33 = nn.Conv2d(48, 48, 3, 1, 1)


        self.corse_conv1 = nn.Sequential(Depthwise_separable_conv(192, 192, kernel_size=3, padding=1, bias=False))
        self.corse_conv2 = nn.Sequential(Depthwise_separable_conv(96, 96, kernel_size=3, padding=1, bias=False))
        self.corse_conv3 = nn.Sequential(Depthwise_separable_conv(48, 48, kernel_size=3, padding=1, bias=False))
    ...
    def forward(self,x):
        ...
        x_down4 = self.relu(self.conv11(x_down4))+x_down4
        x_down4 = self.corse_conv1(x_down4)
        x_level3_mix = self.combine_convs1(torch.cat((x_down4,x1),dim=1))+x1
        ...
        x_down3 = self.relu(self.conv22(x_down3))+x_down3
        x_down3 = self.corse_conv2(x_down3)
        x_level2_mix = self.combine_convs2(torch.cat((x_down3,x_up1),dim=1))+x_up1

        x_up1 = self.up2(x_level2_mix)
        x_up1 = self.conv2(x_up1)

        # x_level2_mix = self.mix3(x_down2, x_up1)
        x_down2 = self.relu(self.conv33(x_down2))+x_down2
        x_down2 = self.corse_conv3(x_down2)
        x_level2_mix = self.combine_convs3(torch.cat((x_down2,x_up1),dim=1))+x_up1
